...
(defn not-aborted? [{:keys [active-requests]} abort-id]
  (contains? @active-requests abort-id))
...
(defn mock-http-server
  ...
  (merge options
         {:active-requests (atom #{})
          :transmit! (fn transmit! [{:keys [active-requests] :as this} {:keys [::txn/ast ::txn/result-handler ::txn/update-handler] :as send-node}]
                       (let [edn (eql/ast->query ast)
                         ...
                         (when abort-id
                           (swap! active-requests conj abort-id))
                         (try
                           (async/go
                            (let [result (async/<! (parser edn))]
                              (if (not-aborted? this abort-id)
                                ...
                                (do
                                  ...
                                  (ok-handler {:status-text "Cancelled"
                                               ::txn/aborted? true})))))
                           (catch :default e
                             (error-handler {:transaction edn :status-code 500})))))
          :abort! (fn abort! [{:keys [active-requests] :as this} id]
                    (log/debug "Aborting" id)
                    (when (not-aborted? this id)
                      (swap! active-requests disj id)))}))
...
(defn person->option [{:person/keys [id name] :as person}]
  #js {:label name
       :value id
...
(defsc PersonSelector [this props {:keys [selected]}]
  {::select-sugar/option-class Person
   ...
   :initial-state (fn [_] {:ui/select (comp/get-initial-state select-sugar/Select {::select-sugar/id "select"})
                           :ui/options []
                           :ui/selected nil
                           :person-selector/id (gensym)})
   :ident :person-selector/id
   :query [{:ui/select (comp/get-query select-sugar/Select)}
           {:ui/options (comp/get-query Person)}
           :person-selector/id]
   :css-include [select-sugar/Select]
   :componentDidMount (fn [this]
                        (let [{:person-selector/keys [id]} (comp/props this)]
                          (select-sugar/start-select! this {::select-sugar/results-actor [:person-selector/id id]
                                                            ::select-sugar/id id})))
   :componentWillUnmount (fn [this]
                           (let [{:person-selector/keys [id]} (comp/props this)]
                             (select-sugar/stop-select! this id)))}
  (log/debug "SELECTED" (:ui/selected props))
  (select-sugar/ui-select
   (comp/computed (:ui/select props) {::select-sugar/uism-id :select-person
                                      ::select-sugar/selected (some-> selected
                                                                      person->option)
...
(defsc FulcroDemo [this props]
  {:initial-state (fn [_] {:id "autocomplete"
   ...
   :ident :id
   :query [:id
           {:selected-person (comp/get-query Person)}
           {:ui/person-selector (comp/get-query PersonSelector)}]
   :css [[:.wrapper {:width "100%"}]]}
  (dom/div :.wrapper
           (ui-person-selector
            (:ui/person-selector props)
            {:selected (:ui/selected props)})))

(ws/defcard kitchensink-demo
