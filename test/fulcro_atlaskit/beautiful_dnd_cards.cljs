...
(defmutation move-sale [{:keys [sale-id
                                from-funnel
                                from-funnel-index
                                to-funnel
                                to-funnel-index]}]
  (action [{:keys [state]}]
          (when-not (and (= from-funnel-index to-funnel-index)
            ...
            (swap! state update-in (conj to-funnel :sales-board.funnel-stage/sales) fa-utils/vec-add to-funnel-index sale-id))))
...
(defsc Sale [this props {::keys [index]}]
  {:query [:sale/id
   ...
   :css [[:.sale {:border "3px solid red"
                  ...
                  :width "200px"}]]}
  (dnd/ui-draggable
   {:draggableId (str (comp/get-ident this)) :index index}
   (fn [provided]
...
(defsc FunnelStage [this {:sales-board.funnel-stage/keys [name id sales]}]
  {:query [:sales-board.funnel-stage/id
   ...
   :css [[:.wrapper {:background "#ccc"
                     :padding "5px 10px"
                     :margin "10px"}]]}
  (dnd/ui-droppable {:droppableId (str (comp/get-ident this))}
...
(defsc FulcroDemo [this {:root/keys [funnel-stages]}]
  ...
  (dnd/ui-drag-and-drop-context {:onDragEnd (fn [result-js]
                                              (let [result (->clj result-js)]
                                                (comp/transact! this `[(move-sale ~{:sale-id (reader/read-string (:draggableId result))
