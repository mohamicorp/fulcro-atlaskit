...
(def format-options
  {:bold {::shortcut ["mod" "b"]
          ::format "bold"
          ::type ::mark}
   :italic {::shortcut ["mod" "i"]
            ::format "italic"
            ::type ::mark}
   :underline {::shortcut ["mod" "u"]
               ::format "underline"
               ::type ::mark}
   :strikethrough {::shortcut ["mod" "shift" "s"]
                   ::format "strikethrough"
                   ::type ::mark}
   :code {::shortcut ["mod" "shift" "m"]
          ::format "code"
          ::type ::mark}
   :superscript {::type ::mark
                 ::format "superscript"}
   :subscript {::type ::mark
               ::format "subscript"}
   :paragraph {::type ::block
               ::format "paragraph"}
   :heading-1 {::type ::block
               ::format "heading-1"
               ::shortcut ["mod" "alt" "1"]}
   :heading-2 {::type ::block
               ::format "heading-2"
               ::shortcut ["mod" "alt" "2"]}
   :heading-3 {::type ::block
               ::format "heading-3"
               ::shortcut ["mod" "alt" "3"]}
   :heading-4 {::type ::block
               ::format "heading-4"
               ::shortcut ["mod" "alt" "4"]}
   :heading-5 {::type ::block
               ::format "heading-5"
               ::shortcut ["mod" "alt" "5"]}
   :heading-6 {::type ::block
               ::format "heading-6"
               ::shortcut ["mod" "alt" "6"]}
   :numbered-list {::type ::block
                   ::format "numbered-list"
                   ::shortcut ["mod" "alt" "7"]}
   :bullet-list {::type ::block
                 ::format "bullet-list"
                 ::shortcut ["mod" "alt" "8"]}
   :list-item {::type ::block
               ::format "list-item"}
   :blockquote {::type ::block
                ::format "blockquote"}})

(def list-types #{(get-in format-options [:numbered-list ::format])
                  (get-in format-options [:bullet-list ::format])})

(def hotkeys (reduce
               (fn [m {::keys [format shortcut type]}]
                 (cond-> m
                   shortcut (assoc (str/join "+" shortcut) [type format])))
               {}
               (vals format-options)))

(defn is-hotkey [hot-key event]
  (isHotkey hot-key event))
...
(defn list-type? [format]
  (contains? list-types format))
...
(defn toggle-block [editor format]
  (let [active? (block-active? editor format)
    ...
    (.unwrapNodes Transforms editor #js {:match #(list-type? (gobj/get % "type"))
                                         :split true})

    (.setNodes Transforms editor
      #js {:type (cond
                   ;; When active disable
    ...
    (when (and block-list-type? (not active?))
      (let [block #js {:type format :children []}]
        (.wrapNodes Transforms editor block)))))

(defn mark-active? [editor format]
  (let [marks (.marks Editor editor)]
    (when marks
      (true? (gobj/get marks format)))))

(defn toggle-mark [editor format]
  (if (mark-active? editor format)
    (.removeMark Editor editor format)
    (.addMark Editor editor format true)))
...
(defn format-option-active? [editor format-option]
  (let [{::keys [format]
         format-type ::type} (get format-options format-option)]
    (case format-type
...
(defsc MoreFormattingOption [this {:keys [label format on-select]}]
  ...
  (comp/with-parent-context this
    (let [editor (useSlate)
          {::keys [shortcut]} (get format-options format)]
      (menu/ui-button-item
        #js {:isSelected (format-option-active? editor format)
             :onMouseDown (fn [e]
                            (.preventDefault e)
                            (toggle-format-option editor format)
                            (when on-select
                              (on-select)))
             :elemAfter (when shortcut
                          (ui-shortcut-lozenge {:shortcut shortcut}))}
        label))))

(defsc Separator [this _]
  {:css [[:.separator {:width "1px"
                       :height "24px"
...
(defsc AtlasEditor [this {:keys [editor value on-change]}]
  {:css-include [Toolbar Separator ShortcutLozenge TooltipShortcutLozenge]
   ...
   :css [[:.wrapper {:border-color colors/N40
                     :border-radius "3px"
                     :border-width "1px"
                     :border-style "solid"
                     :min-width "272px"
      ...
      (ui-toolbar {}
        (popup/ui-popup
          {:isOpen (comp/get-state this :text-styles-dropdown-open?)
           ...
           :trigger (fn [triggerProps]
                      (comp/with-parent-context this
                        (tooltip/ui-tooltip {:content (tr "Text styles") :position "top"}
                          (button/ui-atlaskit-button
                            (utils/js-spread
                              triggerProps
                              #js {:appearance "subtle"
           ...
           :content (fn []
                      (comp/with-parent-context this
                        (menu/ui-menu-group {}
                          (menu/ui-section {}
                            (mapv (fn [props] (ui-more-formatting-option (assoc props :on-select #(comp/update-state! this update :text-styles-dropdown-open? not))))
                              [{:format :paragraph
                                :label "Normal text"}
                               {:format :heading-1
        ...
        (ui-separator {})
        (ui-editor-button {:label "Bold"
                           :icon (dom/span :.icon (bold/ui-icon {:label (tr "Make bold")}))
                           :format :bold} )
        (ui-editor-button {:label "Italic"
        ...
        (popup/ui-popup
          {:isOpen (comp/get-state this :more-formatting-open?)
           ...
           :trigger (fn [triggerProps]
                      (comp/with-parent-context this
                        (tooltip/ui-tooltip {:content (tr "More formatting") :position "top"}
                          (button/ui-atlaskit-button
                            (utils/js-spread
                              triggerProps
                              #js {:appearance "subtle"
        ...
        (ui-separator {})
        (ui-editor-button {:label (tr "Bullet list")
                           :icon (dom/span :.icon (bullet-list/ui-icon {:label (tr "Bullet list")}))
                           :format :bullet-list})
        (ui-editor-button {:label (tr "Numbered list")
                           :icon (dom/span :.icon (number-list/ui-icon {:label (tr "Number list")}))
                           :format :numbered-list})
        (ui-separator {})
        (ui-editor-button {:label (tr "Quote")
                           :icon (dom/span :.icon (quote/ui-icon {:label (tr "Quote")}))
                           :format :blockquote}))
      (dom/div :.editable-wrapper
        (ui-editable {:renderLeaf render-leaf
